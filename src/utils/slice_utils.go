...
type SliceOperation[T comparable] struct {
	value []T
}

func NewSliceOperation[T comparable](src []T) *SliceOperation[T] {
...
func ReverseForEach[T comparable](slice []T, f func(index int, value T)) {
...
func Map[T comparable](src []T, mapper func(element T)) {
	for _, v := range src {
		mapper(v)
	}
}
